    ...
    <div className="h-full flex flex-col lg:flex-row gap-6 min-h-0 print:hidden relative">
        ...
        <div className="flex-1 overflow-auto pr-2 scrollbar-hide">
          {loading ? (
             <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] animate-pulse">Loading Cathtea Menu...</p>
             </div>
          ) : (
            <motion.div 
              initial="hidden"
                ...
                >
                  ...
                  <div className="px-1">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-lg font-black text-slate-900 tracking-tight font-mono">${p.price.toFixed(2)}</span>
      ...
      `}>
        ...
        <div className="p-8 border-b border-pink-50/50 shrink-0 flex items-center justify-between relative z-10">
          <div>
             <h2 className="text-3xl font-display font-black text-slate-900 tracking-tighter uppercase italic">Batch Monitor</h2>
             <div className="flex items-center gap-2 mt-1">
               <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.4)]"></div>
               <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Registry: Synchronized</span>
             </div>
          </div>
          <button 
            onClick={clearCart} 
        ...
        <div className="flex-1 overflow-auto p-6 space-y-4 scrollbar-hide relative z-10">
          {cart.length === 0 ? (
             <div className="flex flex-col items-center justify-center h-full text-center p-10">
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Registry Empty</p>
                <p className="text-[10px] text-slate-300 mt-3 italic leading-relaxed">Add terminal nodes to begin batch processing and settlement protocols.</p>
             </div>
          ) : (
              ...
              {cart.map(item => (
                ...
                >
                  ...
                  <div className="flex items-center bg-white/50 rounded-xl border border-pink-100 p-1">
                    ...
                    <span className="text-xs font-black w-7 text-center text-slate-700 font-mono">{item.quantity}</span>
        ...
        <div className="p-6 lg:p-10 backdrop-blur-3xl bg-pink-50 border-t border-pink-100 shrink-0 space-y-6 lg:space-y-8 relative z-10 shadow-[0_-20px_50px_rgba(0,0,0,0.02)]">
          <div className="space-y-3 lg:space-y-4">
            <div className="flex justify-between text-[9px] lg:text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 italic">
              <span>Gross Subtotal</span>
              <span className="text-slate-600 font-mono tracking-widest">${subtotal.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-[9px] lg:text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 italic">
              <span>Terminal Tax (12%)</span>
            ...
            <div className="flex justify-between items-center pt-4 lg:pt-8 border-t border-pink-100">
              <span className="text-base lg:text-lg font-black text-slate-900 italic tracking-tight uppercase">Batch Total</span>
              <div className="text-right">
                <span className="text-3xl lg:text-4xl font-black text-pink-600 tracking-widest font-mono">${total.toFixed(2)}</span>
                <p className="text-[8px] lg:text-[9px] text-pink-500 font-black uppercase tracking-widest mt-1 italic">Authorized Flow</p>
              </div>
            </div>
          </div>
          <div className="flex gap-4">
             <button 
               ...
               className="lg:hidden flex-1 py-4 bg-white border border-pink-100 text-slate-600 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-pink-50 transition-all active:scale-95"
             >
               Keep Adding
             </button>
             <motion.button 
        ...
        >
          <div className="flex items-center gap-4">
             ...
             <div className="text-left">
                <p className="text-[10px] font-black uppercase text-pink-500 tracking-widest">Active Batch</p>
                <p className="text-sm font-black italic tracking-tight">{cart.length} Nodes Loaded</p>
             </div>
          </div>
          <div className="text-right">
             <p className="text-xl font-black font-mono tracking-tighter">${total.toFixed(2)}</p>
             <p className="text-[8px] font-black opacity-50 uppercase tracking-widest">Tap to Review</p>
          </div>
        </button>
      </div>
            ...
            >
              <div className="p-6 lg:p-10">
                <div className="flex items-center justify-between mb-6 lg:mb-10">
                  <h3 className="text-lg lg:text-xl font-black text-slate-900 italic uppercase tracking-tight">Protocol Settlement</h3>
                ...
                <div className="bg-slate-900 p-6 lg:p-8 rounded-[1.5rem] lg:rounded-[2rem] mb-6 lg:mb-10 flex flex-col items-center relative overflow-hidden group">
                  ...
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] mb-2 lg:mb-3">Payload Value</p>
                  <p className="text-3xl lg:text-5xl font-black text-white tracking-widest font-mono italic">
                ...
                <div className="space-y-4 lg:space-y-6">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] italic ml-1">Select Capture Method</p>
                  <div className="grid grid-cols-2 gap-4 lg:gap-5">
                    ...
                    >
                      ...
                      <span className="font-black text-[9px] lg:text-[10px] uppercase tracking-widest text-slate-900">Liquid Cash</span>
                    </button>
                    <button 
                    ...
                    >
                      ...
                      <span className="font-black text-[9px] lg:text-[10px] uppercase tracking-widest text-slate-900">Encrypted Card</span>
                    </button>
                  </div>
                </div>
                ...
                {paymentMethod === 'cash' && (
                  <div className="mt-8 space-y-4">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] italic ml-1">Cash Intake</p>
                    <div className="relative group/input">
                      ...
                      <input 
                        ...
                        placeholder="0.00"
                        step="0.01"
                        className="w-full pl-12 pr-6 py-5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:outline-none focus:border-pink-500 focus:bg-white transition-all text-xl font-black font-mono tracking-widest"
                    ...
                    <div className="flex justify-between items-center px-2">
                       <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Protocol Exchange</span>
                       <span className="text-xl font-black text-pink-600 font-mono tracking-tighter italic">${changeAmount.toFixed(2)}</span>
                    </div>
                  </div>
                )}
                ...
                <div className="mt-8 lg:mt-12 space-y-3 lg:space-y-4">
                  {paymentMethod === 'cash' && parseFloat(amountReceived) > 0 && !isAmountSufficient && (
                    <div className="flex items-center justify-center gap-2 py-1">
                      ...
                      <p className="text-[10px] font-black text-rose-500 uppercase tracking-[0.2em] italic">
                        Insufficient Intake Protocol
                      </p>
                    </div>
                  )}
                  <motion.button 
                    ...
                    className="w-full py-5 lg:py-6 bg-slate-900 text-white rounded-[1.5rem] lg:rounded-[2rem] font-black uppercase tracking-[0.2em] text-xs lg:text-sm flex items-center justify-center gap-3 hover:bg-slate-800 transition-all shadow-2xl relative overflow-hidden disabled:opacity-50 disabled:grayscale"
                  >
                    {isProcessing ? (
            ...
            >
              <div className="p-6 lg:p-10 print:p-0">
                <div className="text-center mb-6 lg:mb-10">
                  ...
                  <h3 className="text-xl lg:text-2xl font-black text-slate-900 italic uppercase tracking-tighter">Transmission Successful</h3>
                  <p className="text-[9px] lg:text-[10px] text-pink-500 font-black uppercase tracking-[0.2em] mt-1 lg:mt-2">ID_TAG: {receipt.id}</p>
                ...
                <div className="space-y-4 mb-10 max-h-[200px] overflow-auto scrollbar-hide">
                  {receipt.items.map((item: any) => (
                    <div key={item.id} className="flex justify-between items-center bg-pink-50 p-3 rounded-xl">
                      ...
                      <span className="font-black text-slate-900 font-mono tracking-tighter">${(item.price * item.quantity).toFixed(2)}</span>
                    </div>
                  ))}
                </div>

                <div className="border-t-4 border-double border-pink-100 pt-6 lg:pt-8 space-y-2 lg:space-y-3 mb-6 lg:mb-10">
                  <div className="flex justify-between text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest italic">
                  ...
                  <div className="flex justify-between pt-4 lg:pt-5 border-t border-pink-100">
                    <span className="text-xs lg:text-sm font-black text-slate-900 uppercase tracking-[0.2em] italic">Settlement Total</span>
                    <span className="text-xl lg:text-2xl font-black text-pink-600 font-mono italic tracking-tighter">${receipt.total.toFixed(2)}</span>
                  </div>
                </div>
                ...
                <div className="space-y-3 mb-10 p-5 bg-slate-900 rounded-2xl">
                   ...
                   <div className="flex justify-between text-[10px] font-black text-pink-500 uppercase tracking-widest italic pt-2 border-t border-white/10">
                      <span>Exchange</span>
                ...
                <div className="bg-pink-50 border border-pink-100 p-5 rounded-2xl text-center mb-10">
                  <p className="text-[10px] font-black text-pink-400 uppercase tracking-[0.4em] italic">Method: {receipt.paymentMethod}</p>
                </div>

                <div className="flex gap-3 lg:gap-4 print:hidden">
                  <button 
                    onClick={handlePrint}
                    className="flex-1 py-4 lg:py-5 bg-slate-900 text-white rounded-xl lg:rounded-[1.5rem] font-black uppercase tracking-widest text-[10px] lg:text-xs flex items-center justify-center gap-2 lg:gap-3 hover:bg-slate-800 transition-all shadow-xl active:scale-95"
                  >
                  ...
                  <button 
                    onClick={() => setReceipt(null)}
                    className="px-6 lg:px-8 py-4 lg:py-5 text-slate-400 font-black uppercase tracking-widest text-[9px] lg:text-[10px] hover:text-slate-900 transition-colors"
                  >
                    Done
                  </button>
                </div>
