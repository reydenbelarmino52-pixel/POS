...
export default function Shifts() {
  ...
  if (loading) return <div className="text-slate-400 font-bold italic uppercase tracking-widest p-12">Checking registry...</div>;
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      {!currentShift ? (
        <div className="bg-white rounded-[3rem] p-16 border border-pink-100/50 shadow-2xl flex flex-col items-center text-center shadow-pink-500/5 relative overflow-hidden">
          ...
          <h2 className="text-4xl font-display font-black text-slate-900 mb-4 tracking-tighter uppercase italic">Initialize Node</h2>
          <p className="text-slate-500 mb-12 max-w-sm text-sm font-medium italic leading-relaxed">Verify primary liquid assets and synchronize the terminal for the upcoming operational cycle.</p>
          ...
          <div className="w-full max-w-md space-y-8">
            ...
            <div className="space-y-4 text-left">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-4">Starting Balance / Settlement</label>
            ...
            <motion.button 
              ...
              className="w-full py-6 bg-pink-600 text-white rounded-3xl font-black text-[10px] uppercase tracking-[0.2em] shadow-2xl shadow-pink-200 hover:bg-pink-500 transition-all disabled:bg-slate-100 disabled:text-slate-300 disabled:shadow-none"
            >
              Unseal Terminal Node
            </motion.button>
          </div>
        </div>
      ) : (
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="bg-white p-10 rounded-[3rem] border border-pink-100/50 shadow-xl relative overflow-hidden group shadow-pink-500/5">
                ...
                <div className="flex items-center gap-5 mb-10">
                    ...
                    <div>
                        <h3 className="text-xl font-display font-black text-slate-900 italic tracking-tight uppercase">Node Active</h3>
                        <p className="text-[10px] text-pink-500 font-black uppercase tracking-widest mt-1">Operational since {new Date(currentShift.openTime).toLocaleTimeString()}</p>
                    </div>
                </div>

                <div className="space-y-4">
                    <div className="flex justify-between items-center p-6 bg-[#FAF9F6] rounded-2xl border border-slate-100">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Base Liquid Assets</span>
                        <span className="text-lg font-black text-slate-900 tracking-widest font-mono italic">${currentShift.openingBalance.toFixed(2)}</span>
                    </div>
                    <div className="flex items-center justify-between p-6 bg-emerald-50 border border-emerald-100 rounded-2xl">
                        <div className="flex items-center gap-3">
                           ...
                           <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Registry Synchronized</span>
                        </div>
                        <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest italic font-mono">OK</span>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white p-10 rounded-[3rem] border border-pink-100/50 shadow-xl border-t-4 border-t-rose-500 shadow-pink-500/5">
                <h3 className="text-xl font-display font-black text-slate-900 italic tracking-tight uppercase mb-8">Manual Termination</h3>
                {error && (
                ...
                <div className="space-y-6">
                    <div className="space-y-3">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-4">Final Physical Count</label>
                        <div className="relative group">
                    ...
                    <motion.button 
                        ...
                        className="w-full py-5 bg-slate-900 text-white rounded-[1.5rem] font-black text-[10px] uppercase tracking-[0.2em] transition-all shadow-2xl shadow-slate-200 hover:bg-slate-800 active:scale-95 disabled:bg-slate-100 disabled:text-slate-300 disabled:shadow-none"
                    >
                        Execute Terminal Seal
                    </motion.button>
                    <p className="text-[9px] text-slate-400 text-center font-bold uppercase tracking-widest italic px-8">Variance analysis and historical archiving will trigger immediately.</p>
                </div>
            </div>
          </div>
          ...
          <div className="bg-white p-12 rounded-[4rem] border border-pink-100/30 shadow-sm min-h-[250px] flex flex-col items-center justify-center text-center shadow-pink-500/5 group">
             ...
             <p className="text-[10px] text-pink-400 font-black uppercase tracking-[0.4em] italic mb-2">Real-time Node Monitoring Active</p>
             <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest max-w-xs opacity-60">System is currently validating all inbound transaction tokens across decentralized clusters.</p>
          </div>
        </div>
      )}
    </div>
